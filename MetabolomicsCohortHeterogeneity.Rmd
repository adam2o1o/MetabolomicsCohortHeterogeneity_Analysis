...
knitr::opts_knit$set(root.dir = "C:\\Users\\acha5354\\Desktop\\New Folder\\{SW}\\PhD\\Projects\\200518 - Mass Cytometry Analyses\\BioHEART-CT-CyTOF-Proteomics\\src\\CohortHeterogeneityCode")
```
...
#' Load raw data
#' -----------
clin_dat <- read.csv("data_raw/BioHEART_cohort_heterogeneity_clin_data.csv")

mtb_hruv <- readRDS("data_processed/BioHEART_metabolomics_hRUV_AC.rds") %>%
    assay(., "loessShort_concatenate") %>%
    t() %>%
    as.data.frame() %>%
    mutate(sample_id = row.names(.))

mtb_hruv_batch_dat <- readRDS("data_processed/BioHEART_metabolomics_hRUV_AC.rds") %>%
    colData(.)

mtb_dat_raw <- read.csv("data_raw/BioHEART_metabolomics.csv",
               check.names=FALSE,
               header=FALSE) %>%
  t() %>%
  `colnames<-`(.[1, ]) %>%
...
```{r}
#' Metabolite hRUV normalised data
#' -----------
...
#' Plotting some metabolites across batch
#' --------
rep_samps <- gsub("\\*", "", mtb_hruv$sample_id[grepl("\\*", mtb_hruv$sample_id)])
rep_samps <- mtb_hruv %>%
    mutate(batch = gsub("Batch", "", mtb_hruv_batch_dat$batch_info)) %>%
    mutate(rep = ifelse(grepl("\\*", sample_id), TRUE, FALSE)) %>%
    mutate(sample_id = gsub("\\*", "", sample_id)) %>% 
    group_by(sample_id) %>% 
    summarise(batch=length(unique(batch))) %>% 
    filter(batch ==2) %>% 
    pull(sample_id) 

mtb_df_imp %>%
    `colnames<-`(sub("_raw","",colnames(.))) %>%
    mutate_at("sample", as.character) %>%
    mutate(data_type = "raw") %>%
    bind_rows(
        mtb_wide_dat_std_hruv %>%
            select(sample, any_of(colnames(mtb_hruv))) %>%
            filter(sample %in% mtb_pat_rsk$sample) %>%
            mutate(data_type = "hRUV_normalised") %>%
            mutate_at("sample", as.character)
    ) %>%
    left_join(
        mtb_pat_rsk %>%
    ...
    #     mtb_dat_raw %>% 
    #         filter(sample == "Pool") %>%
    #         select(any_of(c(gsub("_raw", "", colnames(mtb_df_imp)),
    #                         "batch"))) %>% 
    ...
    #     mtb_hruv %>% 
    #         mutate(batch = gsub("Batch", "", mtb_hruv_batch_dat$batch_info)) %>%
    #         filter(grepl("Pool", sample_id)) %>%
    #         pivot_longer(cols=!any_of(c(id_cols, "sample_id")),
    #                      names_to="metabolite",
    #                      values_to="val") %>% 
    #         group_by(metabolite, batch) %>%
    #         mutate(mean_val = mean(val, na.rm=TRUE, trim=0.1),
    ...
    #         select(-c("val", "mean_val")) %>%
    #         ungroup() %>%
    #         pivot_wider(id_cols=any_of(c(id_cols, "sample_id")), 
    #                     names_from="metabolite",
    #                     values_from="val_shift") %>%
    #         mutate(data_type="hRUV_normalised") %>%
    #         select(-sample_id) %>%
    ...
    bind_rows(
        mtb_dat_raw %>% 
            ...
            select(any_of(c(gsub("_raw", "", colnames(mtb_df_imp)),
                            "batch", "sample_id"))) %>%
            mutate_at(vars(-c("sample", "sample_id")), as.numeric) %>%
            mutate(sample = "rep") %>%
            mutate(data_type="raw")
    ) %>%
    mutate(is_replicate=ifelse(sample %in% c("rep"), TRUE, FALSE)) %>%
    select(sample_id, is_replicate, batch, data_type, "3HK", "Histidine", "Serotonin",
           "Arginine", "Tryptophan", "Uridine") %>%
    arrange(as.numeric(batch)) %>%
    mutate(batch = factor(batch, levels=unique(batch))) %>%
    pivot_longer(cols=!c("sample_id", "is_replicate","batch","data_type")) %>%
    ggplot() +
    aes(x=batch, y=value) +
    geom_boxplot() +
    geom_point(data=. %>% 
                    filter(is_replicate),
                aes(col=is_replicate),
               size=1) +
    geom_line(data=. %>% 
                    filter(is_replicate),
                aes(group=sample_id,
    ...
    #                 filter(is_replicate),
    #             aes(col=is_replicate)) +
    facet_wrap(~data_type+name, scales="free", ncol=3)+
    theme_bw()
```

```{r fig.width=10, fig.height=4}
ratio_mean_df <- computePairwiseDiff(
    mtb_df_imp %>%
        select(-sample) %>%
        # For zero values, replace with minimum
        mutate_all(~ifelse(.x == 0, min(.x[.x>0], na.rm = TRUE), .x)) %>%
    ...
    mutate(mean_val = mean(val, na.rm=TRUE, trim=0.1),
           sd_val = sd(val, na.rm=TRUE)) %>%
    distinct(batch, metabolite, mean_val)

ratio_b_rep_df <- computePairwiseDiff(
    mtb_dat_raw %>%
    ...
    mutate(sample_id = mtb_dat_raw %>%
           ...
           batch = mtb_dat_raw %>%
               ...
               as.numeric()) %>%
    pivot_longer(
        cols = !any_of(c(id_cols,"sample_id")),
        names_to = "metabolite",
        values_to = "val"
    ) %>%
    left_join(
        ratio_mean_df,
        by=c("batch"="batch", "metabolite"="metabolite")
    ) %>%
    mutate(val_shift = (val - mean_val)) %>%
    select(-c("val", "mean_val")) %>%
    ungroup() %>%
    pivot_wider(id_cols=any_of(c(id_cols,"sample_id")), 
                names_from="metabolite",
                values_from="val_shift") %>%
    select("sample","sample_id","batch", "Histidine_raw--Serotonin_raw", "3HK_raw--Tryptophan_raw", 
           "Arginine_raw--Uridine_raw") %>%
...
computePairwiseDiff(
    ...
    pivot_wider(id_cols=any_of(id_cols), 
                ...
                values_from="val_shift") %>%
    select(-batch) %>%
    select("sample", "Histidine_raw--Serotonin_raw", "3HK_raw--Tryptophan_raw", 
           "Arginine_raw--Uridine_raw") %>%
    ...
    bind_rows(ratio_b_rep_df) %>%
    pivot_longer(cols=!c("batch","data_type", "sample_id", "sample", "is_replicate")) %>%
    ggplot() +
    aes(x=batch, y=value) +
    geom_boxplot()+
    geom_point(data=. %>% 
    ...
    geom_line(data=. %>% 
                ...
                aes(group=sample_id,
                    col=is_replicate),
              alpha=0.6) +
    facet_wrap(~data_type+name, scales="free")+
    theme_bw()
```
...
mtb_df_ratio %>%
    ...
    select(-sample_id) %>%
    pivot_longer(cols=!c("batch","data_type")) %>%
    ggplot() +
    aes(x=batch, y=value) +
    geom_boxplot() +
    facet_wrap(~data_type+name, scales="free")+
...
```{r}
# Get clinical data to perform lca on
lca_df <- clin_dat %>%
...
## Clustering
```{r}
# Get clinical data to cluster on
clust_df <- clin_dat %>%
...
```{r}
# Load Nemoe data 
load(file="data_processed/mtb_hruv_std_ratio_rmoe_model_l1_0.012_l2_0.08.RData")
```
...
load(file="data_processed/mtb_hruv_std_ratio_t_test_df.RData")
...
save(glm_fit_cv_auc, subchrt_cv_auc_df,
     file="data_processed/mtb_hruv_std_ratio_cv_subgroup_res.RData")   
```


```{r fig.height=9, fig.width=9}
load(file="data_processed/mtb_hruv_std_ratio_cv_subgroup_res.RData")
...
task_val_clin_disc = task_val_clin$clone()
...
p1 <- auc_df %>% 
    ...
    theme(legend.position=c(0.985, 0.025),
        ...
        legend.background = element_rect(fill = "transparent"))+
    labs(x="1 - Specificity", y="Sensitivity",
         subtitle="ROC Averaged from 3-fold CV with 5 Repeats in Discovery",
...
p_clin_fimp_rank <- fimp_mean_rank_df %>% 
    ...
    labs(x="|AUC - 0.5|", y="",
         title="Feature importance from 3-fold CV with 5 reps in Discovery Cohort")
